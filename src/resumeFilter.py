...
# display(Markdown("./instructions/fake_resume.md"))


def build_agents(verbose: bool = True):
    # Initialize the tools (after API keys are set)
    search_tool = SerperDevTool()
    scrape_tool = ScrapeWebsiteTool()
    # Resolve resume path relative to this file to avoid CWD issues
    base_dir = os.path.dirname(__file__)
    resume_path = os.path.join(base_dir, "instructions", "fake_resume.md")
...
def build_tasks(researcher: Agent, profiler: Agent, resume_strategist: Agent, interview_preparer: Agent, inputs: dict):

        # Task for Researcher Agent: Extract Job Requirements
        research_task = Task(
            description=(
                f"Analyze the job posting URL provided {inputs['job_posting_url']} "
        ...
        profile_task = Task(
            description=(
                ...
                f"using the GitHub ({inputs['github_url']}) URL and personal write-up "
                f"({inputs['personal_writeup']}). Utilize tools to extract and "
...
def build_crew(inputs: dict, verbose: int = 2) -> Crew:
    researcher, profiler, resume_strategist, interview_preparer = build_agents(verbose=bool(verbose))
    research_task, profile_task, resume_strategy_task, interview_preparation_task = build_tasks(
        researcher, profiler, resume_strategist, interview_preparer, inputs=inputs
    )
...
def run_topic(inputs: dict):
    ...
    job_application_crew = build_crew(inputs=inputs, verbose=2)
 
    return job_application_crew.kickoff(inputs=inputs)
...
def main():
    ...
    result = run_topic(inputs=job_application_inputs)
