...
def find_best_item_match(requested_item: str, inventory_df: pd.DataFrame) -> tuple:
    """
    Find the best matching item in inventory using fuzzy matching.
    Handles variations like "A4 glossy paper" matching "Glossy paper".
    
    Args:
        requested_item: The item name requested by customer
        inventory_df: DataFrame containing inventory items
    
    Returns:
        tuple: (matched_item_row, confidence_score) or (None, 0) if no match
    """
    requested_lower = requested_item.lower().strip()
    
    # First try exact match (case-insensitive)
    exact_match = inventory_df[inventory_df["item_name"].str.lower() == requested_lower]
    if not exact_match.empty:
        return exact_match.iloc[0], 1.0
    
    # Extract key words from requested item
    requested_words = set(requested_lower.replace('(', ' ').replace(')', ' ').split())
    ...
    for idx, row in inventory_df.iterrows():
        ...
        item_words = set(item_name_lower.replace('(', ' ').replace(')', ' ').split())
        
        # Calculate word overlap
        common_words = requested_words.intersection(item_words)
        if not common_words:
            continue
        
        # Score based on word overlap
        score = len(common_words) / max(len(requested_words), len(item_words))
        
        # Bonus if all item words are in request (item is subset of request)
        if item_words.issubset(requested_words):
            score += 0.5
    ...
    # Only return if confidence is reasonable (at least 40%)
    if best_score >= 0.4:
...
def check_inventory_status(item_name: str, request_date: str) -> str:
    ...
    try:
        ...
        matched_item, confidence = find_best_item_match(item_name, inventory_df)
        
        if matched_item is None:
            return f"Item '{item_name}' not found in inventory catalog."
        
        # Use the actual database name for subsequent operations
        actual_item_name = matched_item["item_name"]
        stock_info = get_stock_level(actual_item_name, request_date)
        current_stock = int(stock_info["current_stock"].iloc[0])
        unit_price = float(matched_item["unit_price"])
        min_stock = int(matched_item["min_stock_level"])
        
        result = f"Item: {actual_item_name}\n"
        if confidence < 1.0:
            result += f"(Matched '{item_name}' to '{actual_item_name}')\n"
...
def generate_customer_quote(items_and_quantities: str, request_date: str) -> str:
    ...
    try:
        ...
        for item_dict in items_list:
            ...
            actual_item_name = matched_item["item_name"]
            unit_price = float(matched_item["unit_price"])
            available_stock = current_inventory.get(actual_item_name, 0)
...
def process_customer_request(request: str, request_date: str) -> str:
    ...
    try:
        prompt = f"""
You are the main orchestrator for Beaver's Choice Paper Company. You coordinate a team of specialized agents to handle customer requests.
...
1. inventory_agent - Manages inventory levels, checks stock, handles reordering, and provides financial status
2. quote_agent - Generates accurate quotes based on customer requests and historical data
3. sales_agent - Processes and finalizes sales transactions

Your Workflow:
1. Analyze the customer request to determine which agent(s) should handle it
2. Delegate tasks to the appropriate specialized agent(s):
   - For inventory inquiries or stock checks → delegate to inventory_agent
   - For quote generation or price inquiries → delegate to quote_agent
   - For completing purchases or finalizing sales → delegate to sales_agent
3. If a task requires multiple steps, coordinate between agents sequentially
4. Synthesize responses from agents into a cohesive final answer for the customer

Task Delegation Guidelines:
- For quote requests: First use quote_agent to search historical quotes and generate quote
- For sales: First check with inventory_agent for availability, then delegate to sales_agent to finalize
- For reordering: Use inventory_agent to check stock levels and place reorders if needed
- After sales, check with inventory_agent if any items need reordering

Now coordinate the agents to handle this request comprehensively.
"""
...
def run_test_scenarios():
    ...
    # Process a limited number of requests to avoid rate limits
    # Change max_requests to process more (up to 20)
    max_requests = min(10, len(quote_requests_sample))
    processed = 0
    ...
    for idx, row in quote_requests_sample.iterrows():
        ...
        print("\n" + "=" * 80)
        print(f"REQUEST {processed+1} of {max_requests}")
        print("=" * 80)
        ...
        # Process request through multi-agent system with retry logic
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_str = str(e)
                if attempt < max_retries - 1 and ("rate" in error_str.lower() or "429" in error_str):
                    print(f"\n⚠️ Rate limit hit, waiting {retry_delay}s before retry {attempt+1}/{max_retries}...")
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    response = f"[Agent delegation active] Request processed with error: {error_str[:100]}"
                    print(f"\n⚠️ {response}")
                    break
        ...
        results.append(
            {
                ...
                "response": response if response else "Processing incomplete",
            }
        )
        
        processed += 1
        
        # Save partial results after each request
        pd.DataFrame(results).to_csv("test_results.csv", index=False)
        print(f"✓ Progress saved to test_results.csv ({processed}/{max_requests})")

        time.sleep(8)  # Increased delay to avoid rate limits

    # Final report
    if results:
        final_date = results[-1]["request_date"]
        ...
        print(f"✓ Cash balance changes documented: {len(set([r['cash_balance'] for r in results]))} unique values")
    else:
